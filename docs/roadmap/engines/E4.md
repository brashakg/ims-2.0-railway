...
> Build from **`features/E4.md`** + **`CORRECTIONS.md`** instead. IGNORE the dead text below: settings reads use **`get_policy`/`get_policies`** (E2 now exists), never `resolve_setting`; E4 does NOT own a `_DEFAULT_TIERS` constant (E2's `refund.tier.*` registry default IS the fallback). `approve()` is a single atomic `find_one_and_update({request_id,status:REQUESTED,consumed:false})` minting the token in the same op; add a PIN brute-force throttle (`pin_attempts`).
