...
> E1 is DONE (PR #563; `backend/api/services/money_guard.py`). This is the un-folded original contract. Build/reference from **`features/E1.md`** + **`CORRECTIONS.md` P0-1** instead. IGNORE the dead text below: the `money_accounts` SoR + cross-collection "atomic dual-write" (impossible on standalone Mongo) is CANCELLED; new balance types are per-type single-doc collections (R1); audit via **`AuditRepository.create`**, never `append_audit_entry`.
