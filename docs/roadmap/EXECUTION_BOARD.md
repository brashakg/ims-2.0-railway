...
|---|---|---|---|---|
| SC | Scorecard + slab-incentive | [#582](https://github.com/brashakg/ims-2.0-railway/pull/582) | `feat/SC-scorecard` | One `scorecard_engine.py` extracted from the existing points/payout calculators (object-identity reuse test — no fork) + Product-Incentive kicker + E2 resolution + payroll fed from the LOCKED snapshot only. **Adversarial: no P0/P1 — the payroll double-count was re-verified FALSE** (run loop assigns one source + `$set` overwrites; SC-T8b pins it); multiplier 1.1@14%; RBAC clean. **1 P2 FIXED:** self-loggable kicker amount server-bounded ±1L (was unbounded, feeds payroll). 17 tests + rbac 29; smoke 1006; E/F + tsc/vite clean. P2 follow-up: post-lock kicker 409. |
