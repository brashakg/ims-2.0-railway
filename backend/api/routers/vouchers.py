...
async def redeem_voucher(
    ...
    cannot both succeed — the second finds no matching document and we 400
    it. No read-modify-write window, no double-spend.
    """
    db = _get_db()
    coll = _coll(db)
    if coll is None:
    ...
    if updated is None:
        # Disambiguate the failure with a follow-up read so the cashier
        # gets a precise message. This read is purely for messaging — the
        # atomic guard above already prevented any spend.
        raise HTTPException(status_code=400, detail=_redeem_failure_reason(coll, code_u, amount))
