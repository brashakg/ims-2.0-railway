...
async def create_follow_up(
    request: CreateFollowUpRequest, current_user: dict = Depends(get_current_user)
):
    """
    ...
    follow_up_id = (
        f"FU-{datetime.now().strftime('%Y%m%d')}-{str(uuid.uuid4())[:8].upper()}"
    )
...
async def auto_generate_follow_ups(
    ...
    try:
                            ...
                            follow_ups_collection.insert_one(
                                {
                                    ...
                                    "customer_phone": prescription.get(
                                        "customer_phone"
                                    ),
                                    "store_id": store_id,
                                    "type": "prescription_expiry",
        ...
        raise HTTPException(
            status_code=500, detail="Error generating follow-ups. Please try again."
        )
...
async def get_follow_up_summary(
    ...
    due_today = collection.count_documents(
        {"store_id": store_id, "status": "pending", "scheduled_date": {"$lte": today}}
    )
