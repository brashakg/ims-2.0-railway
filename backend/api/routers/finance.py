...
async def get_gst_reconciliation(
    ...
    # restrict to finance admins so a single-store STORE_MANAGER/AREA_MANAGER
    # cannot read every entity's GST position.
    _require_finance_admin(current_user)
    db = _get_db()
    now = now_ist()
    m = month or now.month
...
async def get_tally_sales_jv(
    ...
    # Org-wide sales-voucher export is an accounting function -- restrict to
    # finance admins so a store-level role can't export all stores' sales JV.
    _require_finance_admin(current_user)
    db = _get_db()
