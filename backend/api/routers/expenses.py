...
async def submit_expense(
    ...
    if expense_repo:
        ...
        if existing.get("status") != "DRAFT":
            raise HTTPException(status_code=400, detail="Expense is not in draft status")
...
async def request_advance(
    ...
    if advance_repo:
        advance_repo.create({
            ...
            "expected_settlement_date": advance.expected_settlement_date.isoformat() if advance.expected_settlement_date else None,
            "status": "PENDING",
            "created_at": datetime.now().isoformat()
        })
...
async def approve_advance(
    ...
    if advance_repo:
        ...
        advance_repo.update(advance_id, {
            "status": "APPROVED",
            "approved_by": current_user.get("user_id"),
            "approved_at": datetime.now().isoformat()
        })
...
async def disburse_advance(
    ...
    if advance_repo:
        ...
        if existing.get("status") != "APPROVED":
            raise HTTPException(status_code=400, detail="Advance must be approved first")
    ...
    return {"message": "Advance disbursed", "advance_id": advance_id, "reference": reference}
...
async def settle_advance(
    ...
    if advance_repo:
        ...
        if existing.get("status") != "DISBURSED":
            raise HTTPException(status_code=400, detail="Advance must be disbursed first")
