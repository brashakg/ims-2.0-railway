...
def test_helper_single_frame_item_5pct():
    """FRAME at ₹1000 → 5% GST → tax 50, taxable 1000."""
    from api.routers.orders import _compute_per_category_gst
...
def test_helper_sunglass_item_18pct():
    """SUNGLASS at ₹1000 → 18% GST → tax 180."""
    from api.routers.orders import _compute_per_category_gst
    out = _compute_per_category_gst(
        [{"item_total": 1000.0, "category": "SUNGLASSES"}], 0,
    )
    assert out["taxable"] == 1000.0
...
def test_helper_mixed_categories():
    """₹1000 FRAME (5%) + ₹500 SUNGLASS (18%) → tax = 50 + 90 = 140."""
    from api.routers.orders import _compute_per_category_gst
    items = [
    ...
    assert out["taxable"] == 1500.0
    assert out["tax"] == 140.0
    # Dominant rate = highest-revenue rate = 5% (FRAME bucket has more)
    assert out["dominant_rate"] == 5.0


def test_helper_cart_discount_applied_before_tax():
    """₹1000 FRAME (5%) − 10% cart discount → taxable 900, tax 45."""
    from api.routers.orders import _compute_per_category_gst
    items = [{"item_total": 1000.0, "category": "FRAME"}]
    out = _compute_per_category_gst(items, 10.0)
    assert out["subtotal"] == 1000.0
    assert out["taxable"] == 900.0
    assert out["tax"] == 45.0
...
def test_helper_falls_back_to_18pct_for_unknown_category():
    ...
    from api.routers.orders import _compute_per_category_gst
    out = _compute_per_category_gst(
        [{"item_total": 1000.0, "category": "ACCESSORIES"}], 0,
    )
    assert out["dominant_rate"] == 18.0
    assert out["tax"] == 180.0
...
def _frame_item(unit_price=1000.0):
    return {
        "product_id": "custom-frame", "product_name": "Test Frame",
        "item_type": "FRAME", "category": "FRAME",
...
def _sunglass_item(unit_price=2000.0):
    return {
        "product_id": "custom-sg", "product_name": "Test Sunglass",
        "item_type": "SUNGLASSES", "category": "SUNGLASSES",
        "quantity": 1, "unit_price": unit_price,
    }
...
def test_create_order_grand_total_is_nonzero(client, auth_headers, patched_orders):
    ...
    body = resp.json()
    # Frame at ₹1000 + 5% GST = 1050
    assert body["grand_total"] == 1050.0


def test_create_order_mixed_categories_per_cat_gst(
    client, auth_headers, patched_orders
):
    """Mixed cart: FRAME (5%) + SUNGLASS (18%). Tax = 50 + 360 = 410."""
    resp = _post_order(client, auth_headers, [
        _frame_item(1000.0), _sunglass_item(2000.0),
    ])
    assert resp.status_code in (200, 201), resp.text
    body = resp.json()
    # subtotal = 1000 + 2000 = 3000
    # tax = 1000*0.05 + 2000*0.18 = 50 + 360 = 410
    # grand_total = 3000 + 410 = 3410
    assert body["grand_total"] == 3410.0


def test_create_order_persists_total_discount(
    client, auth_headers, patched_orders
):
    """The Module (iii) payout aggregator reads `total_discount` off
    the order doc — verify it gets stamped on create."""
    resp = _post_order(
        client, auth_headers, [_frame_item(1000.0)],
...
def test_add_order_item_uses_per_category_gst(
    client, auth_headers, patched_orders
):
    ...
    saved = next(
        d for d in patched_orders["order_repo"].collection.docs
        if d.get("order_id") == order_id
    )
    # subtotal = 3000, tax = 50 + 360 = 410
    assert saved["subtotal"] == 3000.0
    assert saved["tax_amount"] == 410.0
    assert saved["grand_total"] == 3410.0
...
def test_remove_order_item_recomputes_per_category(
    ...
    sg_item_id = next(i["item_id"] for i in saved["items"] if i["category"] == "SUNGLASSES")

    resp = client.delete(
        f"/api/v1/orders/{order_id}/items/{sg_item_id}", headers=auth_headers,
    )
    ...
    assert saved["tax_amount"] == 50.0
    assert saved["grand_total"] == 1050.0
...
def test_payout_auto_derive_reads_grand_total_and_created_at(
    ...
    orders_coll.insert_one({
        "_id": "ord-in1", "order_id": "ord-in1",
        "store_id": "BV-TEST-01", "status": "CONFIRMED",
        "grand_total": 5000.0, "total_discount": 200.0,
        "created_at": in_window,
    })
    orders_coll.insert_one({
        "_id": "ord-in2", "order_id": "ord-in2",
        "store_id": "BV-TEST-01", "status": "DELIVERED",
        "grand_total": 7000.0, "total_discount": 100.0,
        "created_at": in_window,
    })
    orders_coll.insert_one({
        "_id": "ord-out1", "order_id": "ord-out1",
        "store_id": "BV-TEST-01", "status": "CONFIRMED",
        "grand_total": 99999.0, "total_discount": 0,
        "created_at": out_window,  # previous month — must not be summed
    })
    orders_coll.insert_one({
        "_id": "ord-cancel", "order_id": "ord-cancel",
        "store_id": "BV-TEST-01", "status": "CANCELLED",
        "grand_total": 50000.0, "total_discount": 0,
    ...
    fake_db.get_collection("incentive_settings").insert_one({
        "_id": "BV-TEST-01", "store_id": "BV-TEST-01",
