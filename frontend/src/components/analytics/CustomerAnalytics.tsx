    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-3xl font-bold text-green-600">${(analytics.totalRevenue / 1000).toFixed(0)}k</p>
          <p className="text-xs text-gray-500 mt-2">
            Avg: ${analytics.averageCustomerValue.toFixed(0)}
          </p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-sm font-medium text-gray-600 mb-2">Repeat Rate</p>
          <p className="text-3xl font-bold text-blue-600">{analytics.repeatPurchaseRate}%</p>
          <p className="text-xs text-gray-500 mt-2">
            Churn: {analytics.churnedCustomers}
          </p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-sm font-medium text-gray-600 mb-2">Monthly Growth</p>
          <p className={clsx('text-3xl font-bold', analytics.monthlyGrowth >= 0 ? 'text-green-600' : 'text-red-600')}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-6 py-3 text-right font-semibold text-gray-900">Orders</th>
                <th className="px-6 py-3 text-right font-semibold text-gray-900">Total Spent</th>
                <th className="px-6 py-3 text-center font-semibold text-gray-900">Retention</th>
                <th className="px-6 py-3 text-center font-semibold text-gray-900">Last Order</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                filteredMetrics.map(metric => (
                  <tr key={metric.customerId} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-3">
                      <div>
                        <p className="font-medium text-gray-900">{metric.customerName}</p>
                        <p className="text-xs text-gray-500">{metric.email}</p>
                      </div>
                    </td>
                    <td className="px-6 py-3">
                    ...
                    <td className="px-6 py-3 text-right">
                      <span className="font-medium text-gray-900">{metric.totalOrders}</span>
                    </td>
                    <td className="px-6 py-3 text-right">
                      <span className="font-medium text-gray-900">${metric.totalSpent.toFixed(2)}</span>
                    </td>
