    ...
    <div className="space-y-2">
      {items.map(item => (
        <div key={item.id} className="bg-white rounded-lg border border-red-200">
          ...
          {/* Expanded Details */}
          {expandedId === item.id && (
            <div className="border-t border-red-200 p-4 bg-red-50/50">
              {/* Change History */}
              ...
              {changes.map((change, i) => {
                ...
                return (
                  <div key={i} className="relative">
                    {/* Timeline Dot */}
                    <div className={clsx(
                      'absolute -left-12 top-1 w-8 h-8 rounded-full border-4 border-white flex items-center justify-center text-xs font-bold',
