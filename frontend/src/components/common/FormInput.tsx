...
export function FormInput({
  ...
  const baseInputClasses = clsx(
    ...
    displayError
      ? 'border-red-300 bg-red-50 text-gray-900 placeholder-red-300 focus:ring-red-500'
      : isValid && showSuccess
        ? 'border-green-300 bg-green-50 text-gray-900 placeholder-gray-400 focus:ring-green-500'
  ...
  return (
    <div className={clsx('space-y-2', containerClassName)}>
      ...
      <div className="relative group">
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
          ...
          {TrailingIcon && type !== 'password' && (
            <TrailingIcon className="w-4 h-4 text-gray-500 pointer-events-none" />
          )}
        </div>
      </div>
      ...
      <div className="min-h-[1rem] text-xs space-y-0.5">
        ...
        {!displayError && hint && (
          <p id={`${inputId}-hint`} className="text-gray-500">
            {hint}
          </p>
        )}
...
export function FormSelect({
  ...
  return (
    <div className={clsx('space-y-2', containerClassName)}>
      {label && (
        <label htmlFor={inputId} className={clsx('block text-sm font-medium text-gray-700', labelClassName)}>
          {label}
      ...
      <div className="relative">
        {Icon && <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none">
          <Icon className="w-4 h-4" />
        </div>}
        ...
        <select
          ...
          className={clsx(
            ...
            sizeClasses[size],
            error ? 'border-red-300 bg-red-50 text-red-900' : 'border-gray-300 bg-white text-gray-900',
            Icon && 'pl-9',
            'pr-9',
            selectClassName,
            className
