...
const statusBg = {
  ...
  on_hold: 'bg-blue-50 border-blue-200',
};
...
export function ApprovalWorkflow({
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200">
      {/* Header */}
      <div className={clsx('p-6 border-b border-gray-200', statusBg[request.status])}>
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <span className={clsx('text-xs px-2 py-1 rounded', {
              ...
              'bg-orange-100 text-orange-700': request.priority === 'high',
              'bg-yellow-100 text-yellow-700': request.priority === 'normal',
              'bg-blue-100 text-blue-700': request.priority === 'low',
            })}>
            ...
            <div key={step.id} className="relative">
              ...
              <button
                ...
                className={clsx(
                  ...
                  step.status === 'on_hold' && 'border-blue-200 bg-blue-50'
                )}
              >
                  ...
                  {step.status === 'pending' && canApprove && request.currentLevel === index && (
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 px-4 py-2 border border-red-300 text-red-600 rounded-lg hover:bg-red-50 transition-colors font-medium disabled:opacity-50"
                        disabled={actionLoading}
                      >
      ...
      {showApproveModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowApproveModal(false)}>
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            />
            <div className="flex gap-2">
              <button onClick={() => setShowApproveModal(false)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowRejectModal(false)}>
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            />
            <div className="flex gap-2">
              <button onClick={() => setShowRejectModal(false)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
