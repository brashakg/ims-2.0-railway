...
**Last session summary** (ended 2026-04-21, 38 commits):
...
- **Phase 6.5 — Restored 8/8 Jarvis agents**: User saw only 5 of the 8 agents in production. Two structural fragilities: (1) JARVIS was never registered in the backend — frontend synthesized a `JARVIS_STUB` client-side, masking the gap; (2) `initialize_registry` was straight-line `register_agent(X(db=db))` lines, so any one constructor or import failing on Railway silently took out every agent registered after it (lifespan's outer try/except swallowed the trace). Fixes: new `JarvisCore` agent in [`backend/agents/implementations/jarvis.py`](backend/agents/implementations/jarvis.py), JARVIS row added to `DEFAULT_AGENT_CONFIGS`, registry rewritten with `_safe_register` + lazy `_import_class` so each of the 8 agents is now isolated — one failure logs `[REGISTRY] Failed to register agent 'X': …` with full traceback and the rest still come up. Frontend `JarvisPage.tsx` switched from `[STUB, ...liveAgents]` to a `Map`-based dedupe so the live `jarvis` row from the API wins over the stub, but the stub stays as a transitional fallback during the brief window between Vercel + Railway deploys. 6 regression tests in `backend/tests/test_registry_resilience.py` lock down the 8-agent invariant + the no-cascade contract. Counts unchanged (this is a fix, not a feature).
